  ...
  */
  ...
  if (prev_pts.size() > 0) {
    ...
    if (hasPrediction) {
      ...
      if (succ_num < 10)
        cv::calcOpticalFlowPyrLK(
          prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
    } else
      cv::calcOpticalFlowPyrLK(
        prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
    // reverse check
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("vins_estimator"), "temporal optical flow costs: %fms", t_o.toc());
    // printf("track cnt %d\n", (int)ids.size());
  }
  ...
  if (!_img1.empty() && stereo_cam) {
    ...
    if (!cur_pts.empty()) {
      ...
      // cur left ---- cur right
      cv::calcOpticalFlowPyrLK(
        cur_img, rightImg, cur_pts, cur_right_pts, status, err, cv::Size(21, 21), 3);
      // reverse check cur right ---- cur left
      if (FLOW_BACK) {
        cv::calcOpticalFlowPyrLK(
          rightImg, cur_img, cur_right_pts, reverseLeftPts, statusRightLeft, err, cv::Size(21, 21),
          3);
        for (size_t i = 0; i < status.size(); i++) {
          if (
            status[i] && statusRightLeft[i] && inBorder(cur_right_pts[i]) &&
...
void FeatureTracker::rejectWithF()
{
  if (cur_pts.size() >= 8) {
    ...
    reduceVector(track_cnt, status);
    RCLCPP_DEBUG(
      rclcpp::get_logger("vins_estimator"), "FM ransac: %d -> %lu: %f", size_a, cur_pts.size(),
...
void FeatureTracker::readIntrinsicParameter(const vector<string> & calib_file)
{
  for (size_t i = 0; i < calib_file.size(); i++) {
    RCLCPP_INFO(
      rclcpp::get_logger("vins_estimator"), "reading paramerter of camera %s",
      calib_file[i].c_str());
...
void FeatureTracker::showUndistortion(const string & name)
{
  ...
  for (int i = 0; i < int(undistortedp.size()); i++) {
    ...
    if (
      pp.at<float>(1, 0) + 300 >= 0 && pp.at<float>(1, 0) + 300 < row + 600 &&
...
vector<cv::Point2f> FeatureTracker::ptsVelocity(
  vector<int> & ids, vector<cv::Point2f> & pts, map<int, cv::Point2f> & cur_id_pts,
