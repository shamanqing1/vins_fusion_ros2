  ...
  double td)
{
  ...
  if (
    frame_count < 2 || last_track_num < 20 || long_track_num < 40 ||
  ...
  if (parallax_num == 0) {
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("vins_estimator"), "parallax_sum: %lf, parallax_num: %d", parallax_sum,
      parallax_num);
    RCLCPP_DEBUG(
      rclcpp::get_logger("vins_estimator"), "current parallax: %lf",
...
void FeatureManager::setDepth(const VectorXd & x)
{
  ...
  for (auto & it_per_id : feature) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), "feature id %d , start_frame %d, depth %f
    // ", it_per_id->feature_id, it_per_id-> start_frame, it_per_id->estimated_depth);
...
bool FeatureManager::solvePoseByPnP(
  Eigen::Matrix3d & R, Eigen::Vector3d & P, vector<cv::Point2f> & pts2D,
...
void FeatureManager::initFramePoseByPnP(
