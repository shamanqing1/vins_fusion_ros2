...
void Estimator::processMeasurements()
{
  while (1) {
    ...
    if (!featureBuf.empty()) {
      ...
      header.stamp = rclcpp::Time(feature.first * 1e9);
  ...
  const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> & image, const double header)
{
  ...
  RCLCPP_DEBUG(
    rclcpp::get_logger("vins_estimator"), "%s", marginalization_flag ? "Non-keyframe" : "Keyframe");
  ...
  if (ESTIMATE_EXTRINSIC == 2) {
    ...
    if (frame_count != 0) {
      ...
      if (initial_ex_rotation.CalibrationExRotation(
        ...
        RCLCPP_WARN(
          rclcpp::get_logger("vins_estimator"), "initial extrinsic rotation calib success");
        RCLCPP_WARN_STREAM(
          rclcpp::get_logger("vins_estimator"), "initial extrinsic rotation: " << endl
                                                                               << calib_ric);
        ric[0] = calib_ric;
        RIC[0] = calib_ric;
...
bool Estimator::initialStructure()
{
  ...
  if (!relativePose(relative_R, relative_T, l)) {
    RCLCPP_INFO(
      rclcpp::get_logger("vins_estimator"), "Not enough features or parallax; Move device around");
    return false;
  }
  GlobalSFM sfm;
...
bool Estimator::visualInitialAlign()
{
  ...
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("vins_estimator"), "my R0  " << Utility::R2ypr(Rs[0]).transpose());
...
bool Estimator::failureDetection()
{
  ...
  if (f_manager.last_track_num < 2) {
    RCLCPP_INFO(
      rclcpp::get_logger("vins_estimator"), " little feature %d", f_manager.last_track_num);
    // return true;
  }
  if (Bas[WINDOW_SIZE].norm() > 2.5) {
    RCLCPP_INFO(
      rclcpp::get_logger("vins_estimator"), " big IMU acc bias estimation %f",
      Bas[WINDOW_SIZE].norm());
    return true;
  }
  if (Bgs[WINDOW_SIZE].norm() > 1.0) {
    RCLCPP_INFO(
      rclcpp::get_logger("vins_estimator"), " big IMU gyr bias estimation %f",
  ...
  if (tic(0) > 1)
  {
      RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), " big extri param estimation %d", tic(0) >
  1); return true;
  }
  */
...
void Estimator::optimization()
{
  ...
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ...
    if (
      (ESTIMATE_EXTRINSIC && frame_count == WINDOW_SIZE && Vs[0].norm() > 0.2) ||
      openExEstimation) {
      // RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), "estimate extinsic param");
      openExEstimation = 1;
    } else {
      // RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), "fix extinsic param");
      ...
      if (STEREO && it_per_frame.is_stereo) {
        ...
        if (imu_i != imu_j) {
          ProjectionTwoFrameTwoCamFactor * f = new ProjectionTwoFrameTwoCamFactor(
            ...
            it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td);
          problem.AddResidualBlock(
            f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_Ex_Pose[1],
            para_Feature[feature_index], para_Td[0]);
        } else {
          ProjectionOneFrameTwoCamFactor * f = new ProjectionOneFrameTwoCamFactor(
            ...
            it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td);
          problem.AddResidualBlock(
            f, loss_function, para_Ex_Pose[0], para_Ex_Pose[1], para_Feature[feature_index],
            para_Td[0]);
        }
      }
      f_m_cnt++;
  ...
  RCLCPP_DEBUG(
    rclcpp::get_logger("vins_estimator"), "Iterations : %d",
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    if (last_marginalization_info && last_marginalization_info->valid) {
      ...
      for (int i = 0; i < static_cast<int>(last_marginalization_parameter_blocks.size()); i++) {
        if (
          last_marginalization_parameter_blocks[i] == para_Pose[0] ||
    ...
    RCLCPP_DEBUG(
      rclcpp::get_logger("vins_estimator"), "pre marginalization %f ms", t_pre_margin.toc());
    ...
    if (
      ...
      marginalization_info->preMarginalize();
      RCLCPP_DEBUG(
        rclcpp::get_logger("vins_estimator"), "end pre marginalization, %f ms", t_pre_margin.toc());

      TicToc t_margin;
      RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"), "begin marginalization");
      marginalization_info->marginalize();
      RCLCPP_DEBUG(
        rclcpp::get_logger("vins_estimator"), "end marginalization, %f ms", t_margin.toc());
    ...
    for (auto & it_per_frame : it_per_id.feature_per_frame) {
      ...
      if (imu_i != imu_j) {
        ...
        double tmp_error = reprojectionError(
          Rs[imu_i], Ps[imu_i], ric[0], tic[0], Rs[imu_j], Ps[imu_j], ric[0], tic[0], depth, pts_i,
          pts_j);
        err += tmp_error;
        errCnt++;
