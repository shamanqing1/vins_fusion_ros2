...
ProjectionTwoFrameTwoCamFactor::ProjectionTwoFrameTwoCamFactor(
  ...
  const Eigen::Vector2d & _velocity_i, const Eigen::Vector2d & _velocity_j, const double _td_i,
  const double _td_j)
...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(
  double const * const * parameters, double * residuals, double ** jacobians) const
{
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_i.rightCols<3>() =
        ric2.transpose() * Rj.transpose() * Ri * -Utility::skewSymmetric(pts_imu_i);
...
#if 1
      jacobian_feature = reduce * ric2.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 /
                         (inv_dep_i * inv_dep_i);
#else
    ...
    if (jacobians[5]) {
      ...
      jacobian_td =
        reduce * ric2.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0 +
