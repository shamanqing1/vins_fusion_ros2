...
void MarginalizationInfo::marginalize()
{
  ...
  n = pos - m;
  // RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), "marginalization, pos: %d, m: %d, n: %d,
  // size: %d", pos, m, n, (int)parameter_block_idx.size());
  if (m == 0) {
    valid = false;
  ...
  for (auto it : factors)
  {
      for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
      {
          ...
          int size_i =
  localSize(parameter_block_size[reinterpret_cast<long>(it->parameter_blocks[i])]); Eigen::MatrixXd
  jacobian_i = it->jacobians[i].leftCols(size_i); for (int j = i; j <
  static_cast<int>(it->parameter_blocks.size()); j++)
          {
              int idx_j = parameter_block_idx[reinterpret_cast<long>(it->parameter_blocks[j])];
              int size_j =
  localSize(parameter_block_size[reinterpret_cast<long>(it->parameter_blocks[j])]); Eigen::MatrixXd
  jacobian_j = it->jacobians[j].leftCols(size_j); if (i == j) A.block(idx_i, idx_j, size_i, size_j)
  ...
  */
  ...
  // RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"), "thread summing up costs %f ms",
  // t_thread_summing.toc()); RCLCPP_INFO(rclcpp::get_logger("vins_estimator"), "A diff %f , b diff
  // %f ", (A - tmp_A).sum(), (b - tmp_b).sum());

  // TODO
  Eigen::MatrixXd Amm = 0.5 * (A.block(0, 0, m, m) + A.block(0, 0, m, m).transpose());
  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(Amm);

  // assert_MSG(saes.eigenvalues().minCoeff() >= -1e-4, "min eigenvalue %f",
