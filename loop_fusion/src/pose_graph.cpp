...
int PoseGraph::detectLoop(KeyFrame * keyframe, int frame_index)
{
  ...
  if (DEBUG_IMAGE) {
    ...
    putText(
      compressed_image, "feature_num:" + to_string(feature_num), cv::Point2f(10, 10),
  ...
  if (DEBUG_IMAGE) {
    ...
    if (ret.size() > 0)
      putText(
        loop_result, "neighbour score:" + to_string(ret[0].Score), cv::Point2f(10, 50),
  ...
  if (DEBUG_IMAGE) {
    for (unsigned int i = 0; i < ret.size(); i++) {
      ...
      putText(
        tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score),
    ...
    for (unsigned int i = 1; i < ret.size(); i++) {
      ...
      if (ret[i].Score > 0.015) {
        ...
        if (DEBUG_IMAGE && 0) {
          ...
          cv::Mat tmp_image = (it->second).clone();
          putText(
            tmp_image, "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50),
            cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255));
          cv::hconcat(loop_result, tmp_image, loop_result);
        }
      }
      ...
      for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
        ...
        for (int j = 1; j < 5; j++) {
          if (i - j >= 0 && sequence_array[i] == sequence_array[i - j]) {
            ...
            Vector3d relative_t(
              t_array[i][0] - t_array[i - j][0], t_array[i][1] - t_array[i - j][1],
              t_array[i][2] - t_array[i - j][2]);
            relative_t = q_array[i - j].inverse() * relative_t;
            double relative_yaw = euler_array[i][0] - euler_array[i - j][0];
            ceres::CostFunction * cost_function = FourDOFError::Create(
              relative_t.x(), relative_t.y(), relative_t.z(), relative_yaw, euler_conncected.y(),
              euler_conncected.z());
        ...
        if ((*it)->has_loop) {
          ...
          ceres::CostFunction * cost_function = FourDOFWeightError::Create(
            ...
            euler_conncected.z());
          problem.AddResidualBlock(
            cost_function, loss_function, euler_array[connected_index], t_array[connected_index],
      ...
      for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
        ...
        for (int j = 1; j < 5; j++) {
          if (i - j >= 0 && sequence_array[i] == sequence_array[i - j]) {
            Vector3d relative_t(
              t_array[i][0] - t_array[i - j][0], t_array[i][1] - t_array[i - j][1],
              t_array[i][2] - t_array[i - j][2]);
            Quaterniond q_i_j = Quaterniond(
              q_array[i - j][0], q_array[i - j][1], q_array[i - j][2], q_array[i - j][3]);
            Quaterniond q_i =
            ...
            problem.AddResidualBlock(
              vo_function, NULL, q_array[i - j], t_array[i - j], q_array[i], t_array[i]);
          }
        }
        ...
        if ((*it)->has_loop) {
          ...
          ceres::CostFunction * loop_function = RelativeRTError::Create(
            ...
            relative_q.y(), relative_q.z(), 0.1, 0.01);
          problem.AddResidualBlock(
            loop_function, loss_function, q_array[connected_index], t_array[connected_index],
            q_array[i], t_array[i]);
        }
...
void PoseGraph::savePoseGraph()
{
  ...
  for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
    ...
    fprintf(
      pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d\n",
...
void PoseGraph::loadPoseGraph()
{
  ...
  while (fscanf(
           pFile,
           "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf "
           "%lf %lf %lf %d",
           &index, &time_stamp, &VIO_Tx, &VIO_Ty, &VIO_Tz, &PG_Tx, &PG_Ty, &PG_Tz, &VIO_Qw, &VIO_Qx,
           &VIO_Qy, &VIO_Qz, &PG_Qw, &PG_Qx, &PG_Qy, &PG_Qz, &loop_index, &loop_info_0,
           &loop_info_1, &loop_info_2, &loop_info_3, &loop_info_4, &loop_info_5, &loop_info_6,
    ...
    */
    ...
    KeyFrame * keyframe = new KeyFrame(
      time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints,
