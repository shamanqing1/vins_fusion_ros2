...
{
  // RCLCPP_INFO(rclcpp::get_logger("loop_fusion"), "image_callback!");
  m_buf.lock();
  ...
  if (last_image_time == -1)
    ...
    rclcpp::Time(image_msg->header.stamp).seconds() - last_image_time > 1.0 ||
    rclcpp::Time(image_msg->header.stamp).seconds() < last_image_time) {
...
{
  // RCLCPP_INFO(rclcpp::get_logger("loop_fusion"), "point_callback!");
  m_buf.lock();
...
void margin_point_callback(const sensor_msgs::msg::PointCloud::ConstSharedPtr point_msg)
{
  sensor_msgs::msg::PointCloud point_cloud;
  point_cloud.header = point_msg->header;
  for (unsigned int i = 0; i < point_msg->points.size(); i++) {
...
{
  // RCLCPP_INFO(rclcpp::get_logger("loop_fusion"), "pose_callback!");
  m_buf.lock();
...
{
  // RCLCPP_INFO(rclcpp::get_logger("loop_fusion"), "vio_callback!");
  Vector3d vio_t(
  ...
  while (true) {
    ...
    if (!image_buf.empty() && !point_buf.empty() && !pose_buf.empty()) {
      if (
        ...
        rclcpp::Time(image_buf.back()->header.stamp).seconds() >=
          rclcpp::Time(pose_buf.front()->header.stamp).seconds() &&
        ...
        pose_buf.pop();
        while (!pose_buf.empty()) pose_buf.pop();
        while (rclcpp::Time(image_buf.front()->header.stamp).seconds() <
               rclcpp::Time(pose_msg->header.stamp).seconds())
          image_buf.pop();
        image_msg = image_buf.front();
        image_buf.pop();

        while (rclcpp::Time(point_buf.front()->header.stamp).seconds() <
               rclcpp::Time(pose_msg->header.stamp).seconds())
          point_buf.pop();
    ...
    if (pose_msg != NULL) {
      ...
      if ((T - last_t).norm() > SKIP_DIS) {
        ...
        KeyFrame * keyframe = new KeyFrame(
          rclcpp::Time(pose_msg->header.stamp).seconds(), frame_index, T, R, image, point_3d,
...
void command()
{
  while (1) {
    ...
    if (c == 's') {
      ...
      m_process.unlock();
      printf(
        "save pose graph finish\nyou can set 'load_previous_pose_graph' to 1 in the config file to "
        "reuse it next time\n");
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_vio =
    n->create_subscription<nav_msgs::msg::Odometry>("/vins_estimator/odometry", 2000, vio_callback);

  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr sub_image =
    n->create_subscription<sensor_msgs::msg::Image>(IMAGE_TOPIC, 2000, image_callback);

  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_pose =
    n->create_subscription<nav_msgs::msg::Odometry>(
      "/vins_estimator/keyframe_pose", 2000, pose_callback);

  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_extrinsic =
    n->create_subscription<nav_msgs::msg::Odometry>(
      "/vins_estimator/extrinsic", 2000, extrinsic_callback);

  rclcpp::Subscription<sensor_msgs::msg::PointCloud>::SharedPtr sub_point =
    n->create_subscription<sensor_msgs::msg::PointCloud>(
      "/vins_estimator/keyframe_point", 2000, point_callback);

  rclcpp::Subscription<sensor_msgs::msg::PointCloud>::SharedPtr sub_margin_point =
    n->create_subscription<sensor_msgs::msg::PointCloud>(
      "/vins_estimator/margin_cloud", 2000, margin_point_callback);

  pub_match_img = n->create_publisher<sensor_msgs::msg::Image>("/loop_fusion/match_image", 1000);
  pub_camera_pose_visual =
    n->create_publisher<visualization_msgs::msg::MarkerArray>("/loop_fusion/camera_pose_visual", 1000);
  pub_point_cloud =
    n->create_publisher<sensor_msgs::msg::PointCloud>("/loop_fusion/point_cloud_loop_rect", 1000);
  pub_margin_cloud =
    n->create_publisher<sensor_msgs::msg::PointCloud>("/loop_fusion/margin_cloud_loop_rect", 1000);
